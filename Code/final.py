...
try:
    model = load_model('final_model.keras')  # Updated to use .keras extension
except:
    try:
        model = load_model('best_model.keras')
    except:
        print("Error: Could not find model file. Make sure 'final_model.keras' or 'best_model.keras' exists.")
        exit(1)
...
def get_hand_hist():
    try:
        ...
        print("Error: Could not find histogram file. Run hand calibration first.")
        exit(1)

def get_image_size():
    return 64, 64  # Using standard size from training

image_x, image_y = get_image_size()

# Rest of your code remains the same, starting from keras_process_image...
